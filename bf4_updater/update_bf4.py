#!/usr/bin/env python3
"""CLI: update BF4 firmware (PLDM) via BMC, OR serve a DOCA OS ISO over HTTP.

Exactly one mode per invocation, selected by ``--mode``:

  * ``--mode fwpkg`` : push a ``.fwpkg`` from Artifactory to the BF4 BMC.
                       Requires ``--bf4 <bench>`` and ``--pldm-url <url>``.
  * ``--mode os``    : download a DOCA OS ``.iso`` and serve it over HTTP
                       from a bench so the BMC can mount it as virtual
                       media. Requires ``--http_server_bench <bench>``
                       and ``--iso-url <url>``.

Bench names refer to entries in ``benches.yaml``.
...
if __package__ in (None, ""):
    ...
    from bf4_updater import (
        artifactory, config, downloader, iso_server, redfish_fw,
    )
else:
    from . import (
        artifactory, config, downloader, iso_server, redfish_fw,
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="update_bf4",
        description=(
            "Update BF4 firmware (PLDM via BMC Redfish) OR stage a DOCA OS "
            "ISO on a local HTTP server for manual install via the BMC. "
            "Exactly one mode is run per invocation (see --mode)."
        ),
    ...
    p.add_argument(
        ...
        help=(
            ...
            "'fwpkg' = download a .fwpkg from Artifactory and push it to "
            "the BF4 BMC (needs --bf4 and --pldm-url). "
            "'os' = download a DOCA OS .iso and serve it over HTTP from a "
            "bench (needs --http_server_bench and --iso-url)."
        ),
    )
    p.add_argument(
        "--bf4",
        ...
        help=(
            ...
            "(key under 'benches:' in benches.yaml). Required for --mode fwpkg."
        ),
    )
    p.add_argument(
        "--http_server_bench",
        ...
        help=(
            ...
            "(its 'host' is advertised as the URL). Required for --mode os."
        ),
    )
    p.add_argument(
        "--pldm-url",
        help=(
            ...
            "Required for --mode fwpkg."
        ),
    )
    p.add_argument(
        "--iso-url",
        help="Direct URL to the DOCA OS .iso. Required for --mode os.",
    )
    p.add_argument(
        "--artifactory-token",
        default=None,
        help=(
            "Artifactory identity/access token for urm.nvidia.com (--mode "
            "fwpkg). Sent as a Bearer token. If omitted, falls back to the "
            "ARTIFACTORY_TOKEN / ARTIFACTORY_API_KEY / "
            "ARTIFACTORY_USER+ARTIFACTORY_PASSWORD env vars."
        ),
    )
    p.add_argument(
        "--workdir",
    ...
    p.add_argument(
        ...
        type=int,
        default=8000,
        help="Port the local HTTP server listens on for the ISO (--mode os).",
    )
    p.add_argument(
        "--http-bind",
        default="0.0.0.0",
        help="Bind address for the HTTP server (--mode os).",
    )
    p.add_argument(
        "--http-advertise",
        default=None,
        help=(
            ...
            "(--mode os). Defaults to the --http_server_bench's host."
        ),
    )
    p.add_argument(
        "--no-progress",
...
def main(argv: Optional[list] = None) -> int:
    ...
    if args.mode == "fwpkg":
        if not args.bf4:
            log.error("--mode fwpkg requires --bf4.")
            return 2
        if not args.pldm_url:
            log.error("--mode fwpkg requires --pldm-url.")
            return 2
    elif args.mode == "os":
        if not args.http_server_bench:
            log.error("--mode os requires --http_server_bench.")
            return 2
        if not args.iso_url:
            log.error("--mode os requires --iso-url.")
            return 2
    ...
    try:
        if args.mode == "fwpkg":
            ...
            return _do_firmware(bf4_bench, args, show_progress)
...
def _do_firmware(bench: config.Bench, args: argparse.Namespace, show_progress: bool) -> int:
    ...
    if not artifactory.apply_auth(sess, token=args.artifactory_token):
        log.error(
            "No Artifactory credentials found; urm.nvidia.com will reject the "
            "request with 403. Provide --artifactory-token or set "
            "ARTIFACTORY_TOKEN (Bearer), ARTIFACTORY_API_KEY, or "
